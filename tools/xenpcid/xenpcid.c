    ...
    if ( !result ) {
        ...
        if (result->type == JSON_LIST) {
            ...
            if (result->list) {
                ...
                free_list_rsc(result->list_rsc);
            }
            yajl_gen_array_close(hand);
        } else if (result->type == JSON_STRING) {
            if (result->string) {
                pcid__yajl_gen_asciiz(hand, result->string);
                free(result->string);
            } else
                                       ...
                                       struct pcid__yajl_ctx *ctx_yajl)
{
    ...
    if (dst) {
        ...
        case JSON_MAP: {
            ...
            flexarray_get(dst->map, dst->map->count - 1, (void**)&last);
            last->obj = obj;
            break;
        }
        case JSON_ARRAY:
            flexarray_append(dst->array, obj);
            break;
        default:
            fprintf(stderr,
...
static struct pcid__json_object *process_write_cmd(struct pcid__json_object *resp)
{
    ...
    if (strcmp(dir_id->string, XENPCID_PCI_DEV) == 0) {
        full_path = (char *)pcid_zalloc(strlen(SYSFS_PCI_DEV) +
                                        strlen(pci_path->string) + 1);
        sprintf(full_path, SYSFS_PCI_DEV"%s", pci_path->string);
    } else if (strcmp(dir_id->string, XENPCID_PCIBACK_DRIVER) == 0){
        full_path = (char *)pcid_zalloc(strlen(SYSFS_PCIBACK_DRIVER) +
                                        strlen(pci_path->string) + 1);
        sprintf(full_path, SYSFS_PCIBACK_DRIVER"%s", pci_path->string);
    } else if (strcmp(dir_id->string, SYSFS_DRIVER_PATH) == 0){
        full_path = pci_path->string;
    } else {
        fprintf(stderr, "Unknown write directory %s\n", dir_id->string);
        goto free_pci_info;
    }
    ret = handle_write_cmd(full_path, pci_info->string);
    if (strcmp(dir_id->string, SYSFS_DRIVER_PATH) != 0)
        free(full_path);
    if (ret != 0)
        goto free_pci_info;
...
free_pci_info:
    free(pci_info->string);
free_pci_path:
    free(pci_path->string);
free_dir_id:
    free(dir_id->string);
free_args:
    free_pcid_obj_map(args);
...
static struct pcid__json_object *process_read_hex_cmd(struct pcid__json_object *resp)
{
    ...
    if (strcmp(XENPCID_PCI_DEV, dir_id->string) == 0) {
        full_path = (char *)pcid_zalloc(strlen(SYSFS_PCI_DEV) +
                                        strlen(pci_info->string) + 1);
        sprintf(full_path, SYSFS_PCI_DEV"%s", pci_info->string);
    } else
        full_path = pci_info->string;
...
static struct pcid__json_object *process_exists_cmd(struct pcid__json_object *resp)
{
    ...
    if (!pci_info)
        goto free_pci_path;
    if (strcmp(pci_path->string, XENPCID_PCIBACK_DRIVER) == 0) {
        full_path = (char *)pcid_zalloc(strlen(SYSFS_PCIBACK_DRIVER) +
                                            strlen(pci_info->string) + 1);
        sprintf(full_path, SYSFS_PCIBACK_DRIVER"%s", pci_info->string);
        free(pci_info->string);
    } else
        full_path = pci_info->string;
...
static struct pcid__json_object *process_read_rsc_cmd(struct pcid__json_object *resp)
{
    ...
    ret = handle_read_resources_cmd(pci_info->string, &resources_list);
    free(pci_info->string);
    if (ret < 0)
        goto free_args;
...
static struct pcid__json_object *process_unbind_cmd(struct pcid__json_object *resp)
{
    ...
    ret = handle_unbind_cmd(pci_path->string, pci_info->string, &full_path);
    free(pci_path->string);
    if (ret < 0)
        goto free_pci_info;
...
static struct pcid__json_object *process_reset_cmd(struct pcid__json_object *resp)
{
    ...
    ret = handle_reset_cmd(pci_path->string, pci_info->string);
    free(pci_path->string);
    if (ret < 0)
        goto free_pci_info;
                                ...
                                struct pcid__json_object **result)
{
    ...
    command_name = command_obj->string;
