...
static bool handle_pio(uint16_t port, unsigned int size, int dir)
...
#define paging_mark_pfn_dirty(d, pfn) do {} while (0)
...
static enum io_state hvm_send_buffered_ioreq(struct hvm_ioreq_server *s,
                                             ioreq_t *p)
{
...
enum io_state hvm_send_ioreq(struct hvm_ioreq_server *s, ioreq_t *proto_p,
                             bool buffered)
{
...
unsigned int hvm_broadcast_ioreq(ioreq_t *p, bool buffered)
{
    ...
    FOR_EACH_IOREQ_SERVER(d, id, s)
    {
        ...
        if ( hvm_send_ioreq(s, p, buffered) == IO_UNHANDLED )
            failed++;
    }
